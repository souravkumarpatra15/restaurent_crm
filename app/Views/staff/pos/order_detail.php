...
function selectPayment(btn) {
  document.querySelectorAll('.payment-btn').forEach(b => { b.classList.remove('btn-primary'); b.classList.add('btn-outline'); });
...
function setAmt(v) { document.getElementById('payAmt').value = v; calcChange(); }
function calcChange() {
...
function processCheckout() {
  const paid = parseFloat(document.getElementById('payAmt').value) || 0;
  if (paid < ORDER_TOTAL) { showToast('Amount is less than total!', 'error'); return; }
  ...
  fetch('<?= base_url('pos/order/checkout') ?>', {
    ...
    body: `${CSRF_NAME}=${CSRF_TOKEN}&order_id=${ORDER_ID}&payments=${encodeURIComponent(JSON.stringify([{method:selectedMethod,amount:paid,reference:''}]))}`
...
function printKotAction() {
  fetch('<?= base_url('pos/order/print-kot') ?>', {
    ...
    body:`${CSRF_NAME}=${CSRF_TOKEN}&order_id=${ORDER_ID}`
  }).then(r=>r.json()).then(d=>showToast(d.success?'KOT sent to kitchen!':(d.error||'Printer error'), d.success?'success':'error'));
}
...
function printBillAction() {
  fetch('<?= base_url('pos/order/print-bill') ?>', {
    method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
    body:`${CSRF_NAME}=${CSRF_TOKEN}&order_id=${ORDER_ID}`
  }).then(r=>r.json()).then(d=>showToast(d.success?'Bill printed!':(d.error||'Printer error'), d.success?'success':'error'));
}
...
function reprintBill() {
  fetch('<?= base_url('pos/order/print-bill') ?>', {
    method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
    body:`${CSRF_NAME}=${CSRF_TOKEN}&order_id=${ORDER_ID}`
  }).then(r=>r.json()).then(d=>showToast(d.success?'Reprinted!':(d.error||'Error'), d.success?'success':'error'));
}
</script>
